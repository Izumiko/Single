...
<head>
    ...
    <link rel="icon" href="<?= $this->options->favicon ? $this->options->favicon() : $this->options->themeUrl('img/icon.png') ?>" sizes="192x192"/>
    ...
    <?php if ($this->options->background): ?>
        <style>body:before{content: ''; background-image: url(<?= $this->options->background() ?>)}</style>
    <?php endif; ?>

    <?php if ($this->options->custom_css): ?>
        <style><?= $this->options->custom_css() ?></style>
    <?php endif; ?>
    ...
    <?php if ($this->is('post')): ?>
        ...
        <?php if ($hasImage = Single::post_image()): ?>
